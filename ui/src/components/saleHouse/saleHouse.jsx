...
import useStyle from "./saleHouseStyle";
import { connect } from "react-redux";
import userService from "../../services/userService";
import { search } from "../../services/cards";
const SaleHouses = ({ user }) => {
  ...
  const getCards = async () => {
    ...
    setIsLoading(true);
    const { data } = await search("", 0, "sale");
  ...
  const handlePages = async (e, currentPage) => {
    const { data } = await search(searchValue, currentPage - 1, "sale");
    setResults(data.results);
    setCards(data.data);
    window.scrollTo(0, 0);
  };
  ...
  return (
    <div>
      <SearchEngine searchValue={handleSearch} status="sale" />
...
export default connect(mapStateToProps)(SaleHouses);
